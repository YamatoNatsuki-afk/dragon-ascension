# core/CheckpointSystem.gd  ← Autoload (refactorizado)
# Lee CharacterData desde GameStateProvider — sin acoplamiento a DayManager.
extends Node
...
func _evaluate(checkpoint: CheckpointDefinition, data: CharacterData) -> CheckpointResult:
	...
	result.expected_score    = PerformanceEvaluator._expected_score(data.current_day)

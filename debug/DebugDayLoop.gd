...
func begin(data: CharacterData) -> void:
...
func _on_day_action_resolved(action: DayAction, result: DayActionResult) -> void:
	# Registrar stats entrenados hoy para detección de fatiga mañana
...
func _on_day_ended(day_number: int, _result: DayActionResult) -> void:
	...
	var game_over := _character_data.current_day > 100
...
# ─────────────────────────────────────────────
# Fatiga — riesgo emergente sin hardcodear reglas
# ─────────────────────────────────────────────
# Si el personaje entrenó el mismo stat ayer, el evento "overtraining"
# aparece en la lista de hoy con su propio peso. El selector puede elegirlo
# o ignorarlo según las probabilidades — nunca es forzado.
...
func _maybe_inject_fatigue_event() -> void:
	...
	# Verificar solapamiento entre stats de ayer y acciones de hoy
...
func _print_character_snapshot() -> void:
	...
	for stat_id: StringName in _character_data.base_stats.keys():
		...
		var priority: float = _character_data.build.stat_priority_weights.get(stat_id, 0.5)
...
func _print_selection_weights(table: Dictionary, chosen: DayAction) -> void:
...
func _print_result(action: DayAction, result: DayActionResult) -> void:
	print("\n  RESULTADO — %s%s:" % [
		action.display_name,

...
var _period_stat_gains: Dictionary = {}
var _period_xp: float = 0.0
var _period_start_day: int = 1
var _period_start_stats: Dictionary = {}
...
func _on_action_resolved(_action, result) -> void:  # _action: DayAction, result: DayActionResult
...
func print_milestone_report(day_number: int) -> void:
	var days_in_period: int = day_number - _period_start_day + 1
	print("\n")
	print("  ┌─────────────────────────────────────────┐")
	...
	for stat_id: StringName in _period_stat_gains.keys():
		...
		var trend: String  = _trend_arrow(delta)
	...
	if _days_tracked > 0 and _total_xp > 0.0:
		var xp_per_day_avg: float = _total_xp / _days_tracked
		var days_left: int        = 100 - day_number
		var projected_xp: float   = _total_xp + xp_per_day_avg * days_left
...
func print_checkpoint_report(result: CheckpointResult) -> void:
	...
	var build: String = result.build_identity.display_name if result.build_identity else "Desconocido"
	...
	for i in range(mini(2, scores.size())):
		...
		var bar: String        = _mini_bar(s, 1.0, 10)
